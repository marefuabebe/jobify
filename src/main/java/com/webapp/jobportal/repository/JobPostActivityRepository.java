package com.webapp.jobportal.repository;

import com.webapp.jobportal.entity.IRecruiterJobs;
import com.webapp.jobportal.entity.JobPostActivity;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Modifying;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;
import org.springframework.transaction.annotation.Transactional;

import java.time.LocalDate;
import java.util.List;

public interface JobPostActivityRepository extends JpaRepository<JobPostActivity, Integer> {

        @Query(value = " SELECT COUNT(s.user_id) as totalCandidates,j.job_post_id,j.job_title,j.description_of_job,j.posted_date,j.status,j.severity,j.moderation_note as moderationNote,l.id as locationId,l.city,l.state,l.country,c.id as companyId,c.name FROM job_post_activity j "
                        +
                        " inner join job_location l " +
                        " on j.job_location_id = l.id " +
                        " INNER join job_company c  " +
                        " on j.job_company_id = c.id " +
                        " left join job_seeker_apply s " +
                        " on s.job = j.job_post_id " +
                        " where j.posted_by_id = :recruiter " +
                        " GROUP By j.job_post_id", nativeQuery = true)
        List<IRecruiterJobs> getRecruiterJobs(@Param("recruiter") int recruiter);

        @Query(value = "SELECT * FROM job_post_activity j INNER JOIN job_location l on j.job_location_id=l.id  WHERE j"
                        + ".job_title LIKE %:job%"
                        + " AND (l.city LIKE %:location%"
                        + " OR l.country LIKE %:location%"
                        + " OR l.state LIKE %:location%) " +
                        " AND (j.job_type IN(:type)) " +
                        " AND (j.remote IN(:remote)) " +
                        " AND (j.job_category LIKE %:category%) " +
                        " AND (j.status = 'OPEN')", nativeQuery = true)
        List<JobPostActivity> searchWithoutDate(@Param("job") String job,
                        @Param("location") String location,
                        @Param("remote") List<String> remote,
                        @Param("type") List<String> type,
                        @Param("category") String category);

        @Query(value = "SELECT * FROM job_post_activity j INNER JOIN job_location l on j.job_location_id=l.id  WHERE j"
                        + ".job_title LIKE %:job%"
                        + " AND (l.city LIKE %:location%"
                        + " OR l.country LIKE %:location%"
                        + " OR l.state LIKE %:location%) " +
                        " AND (j.job_type IN(:type)) " +
                        " AND (j.remote IN(:remote)) " +
                        " AND (j.job_category LIKE %:category%) " +
                        " AND (j.status = 'OPEN') " +
                        " AND (posted_date >= :date)", nativeQuery = true)
        List<JobPostActivity> search(@Param("job") String job,
                        @Param("location") String location,
                        @Param("remote") List<String> remote,
                        @Param("type") List<String> type,
                        @Param("category") String category,
                        @Param("date") LocalDate searchDate);

        @Modifying
        @Transactional
        @Query(value = "CALL ApproveJobPosting(:jobId, :adminId, :approve, :notes)", nativeQuery = true)
        void approveJobPosting(@Param("jobId") int jobId, @Param("adminId") int adminId,
                        @Param("approve") boolean approve, @Param("notes") String notes);

        @Query(value = "SELECT j.* FROM job_post_activity j JOIN users u ON j.posted_by_id = u.user_id WHERE j.is_active = true AND j.is_approved = true AND u.is_active = true ORDER BY j.posted_date DESC LIMIT 4", nativeQuery = true)
        List<JobPostActivity> getRecentJobs();
}
